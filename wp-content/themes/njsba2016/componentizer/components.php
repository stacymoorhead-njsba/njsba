  ...
  if (is_search()) {
    ...
    if (is_author()) {
      array_unshift($suffixes, 'author');
        ...
        $category_type = get_field( 'category_type', get_queried_object());

        if($category_type == 'publication' || $category_type == 'issue' || $category_type == 'list' || $category_type == 'list_landing'){
          array_unshift($suffixes, $category_type);
        }
